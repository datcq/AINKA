...
{
	$.Controller.extend("Anddev.Popup", {
		defaults: {
		...
		beforeCreateContainer: function(){
			if (this.options.mask == true) {
				this.mask = this.container.anddev_expose({
					color: "#000",
					zIndex:this.options.zIndex
				}).controller();
			};
			if (typeof this.options.mask=="object") {
				this.mask = this.container.anddev_expose(this.options.mask).controller();
			};			
		...
		closeAllPopup: function(ignore){
			...
			for (var i = 0; i < popups.length; i++) {
				...
				if (exist == true) 
					continue;
				var popup = $(popups[i]).controller(Anddev.Popup);
		...
		{
			this.element.trigger("anddev:showPopup");
		...
		onAfterHide:function()
		{
			...
			this.element.trigger("anddev:hidePopup");
		},
		hide: function(){
