$.Controller("drcom.circle",{
		...
		image.onload = function () {
			...
			instance.circle = new Kinetic.Wedge2({
                ...
                onSetAngleDeg:function(deg){
                	...
                	instance.options.change(percent,deg);
                }
			});
	...
	change:function(percent,deg)
	{
	
		var _deg=percent*3.6;
		if(this.options.change(percent)==false)
			return false;

		this.circle.setAngleDeg(_deg);
		this.layer.draw();			
	},
	
	scrollTo:function(percent,_duration,callback)
	{
		var deg=percent*3.6;
		var duration = 2;
		this.animating=true;
